...
XMen_tab = as.matrix(cbind(dat2Men[3],round(dat2Men[4:69],digits=2)))

# disorder names ####
diagnoses=as.matrix(dat2Men[,"diagnosis"])
# rownames(XMen_tab) <- diagnoses
...
nice_diagnoses[nice_diagnoses=="dinpreg"]<-"Chronic+gest diabetes"
...
nice_diagnoses[nice_diagnoses=="hypinpred"]<-"Chronic + gest hyper"
...
XMen_tab<-XMen_tab[!rownames(XMen_tab) %in% drop,]
rownames(XMen_tab)<-NULL
XMen_tab[,1]<-nice_diagnoses
colnames(XMen_tab)<-nice_names
XMen_tab<-as.data.frame(XMen_tab)