...
XIDconf_tab = as.matrix(cbind(dat2IDconf[3],round(dat2IDconf[4:69],digits=2)))

# disorder names ####
diagnoses=as.matrix(dat2IDconf[,"diagnosis"])
# rownames(XIDconf_tab) <- diagnoses
...
raw_names = colnames(XIDconf_tab)
...
XIDconf_tab<-XIDconf_tab[!rownames(XIDconf_tab) %in% drop,]
rownames(XIDconf_tab)<-NULL
XIDconf_tab[,1]<-nice_diagnoses
colnames(XIDconf_tab)<-nice_names
XIDconf_tab<-as.data.frame(XIDconf_tab)