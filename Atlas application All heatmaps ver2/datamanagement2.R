...
X_tab = as.matrix(cbind(dat2[3],round(dat2[4:69],digits=2)))

# disorder names ####
diagnoses=as.matrix(dat2[,"diagnosis"])
# rownames(X_tab) <- diagnoses
...
X_tab<-X_tab[!rownames(X_tab) %in% drop,]
rownames(X_tab)<-NULL
X_tab[,1]<-nice_diagnoses
colnames(X_tab)<-nice_names
X_tab<-as.data.frame(X_tab)